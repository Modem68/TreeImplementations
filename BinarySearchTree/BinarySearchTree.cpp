...
ITERATIVE SOLUTION FOR INSERTING A NODE INORDER
...
bool BinarySearchTree<ItemType>::add(const ItemType& newData)
{
    ...
    else
    {
        ...
        BinaryNode<ItemType>* currentParentNode;
        BinaryNode<ItemType>* currentNode = rootPtr;
        while(currentNode != NULL)
        {
            currentParentNode = currentNode;

            if(currentNode->getItem() > newData)
            {
            ...
            // currentNode = (currentNode->getItem() > newData) ? currentNode->getLeftChildPtr() : currentNode->getRightChildPtr();
        }
      
        // checking for parent value to determine if the new node is a left or right child
        if(currentParentNode->data > newData)
        {
            currentParentNode->setLeftChildPtr(newNode);
        }
        else
        {
            currentParentNode->setRightChildPtr(newNode);
        }
    }
    return true;
}
